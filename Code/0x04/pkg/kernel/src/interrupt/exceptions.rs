...
use crate::memory::*;
use crate::proc::get_process_manager; // 导入进程管理器
...
pub extern "x86-interrupt" fn page_fault_handler(
    ...
    if !crate::proc::handle_page_fault(addr, err_code) {
        // 如果无法处理，则获取当前进程 PID 并 panic
        let current_pid = get_process_manager().current().pid();
        panic!(
            ...
            Stack Frame:\n{:#?}",
            current_pid.0, // .0 获取 u16 值
            err_code,
            addr,
            stack_frame.instruction_pointer, // 添加指令指针信息
            stack_frame
        );
    }
}

...
use x86_64::structures::paging::page::{PageRange, PageSize, PageRangeInclusive};
use x86_64::structures::paging::{mapper::*, *};
use x86_64::{align_up, PhysAddr, VirtAddr};
use xmas_elf::{program, ElfFile};

/// A trait for deallocating frames.
pub trait FrameDeallocator<S: PageSize> {
    fn deallocate_frame(&mut self, frame: PhysFrame<S>);
}

/// Unmap a range of pages and deallocate the corresponding frames.
pub fn unmap_range<D: FrameDeallocator<Size4KiB>>(
    range: PageRangeInclusive,
    mapper: &mut impl Mapper<Size4KiB>,
    dealloc: &mut D,
    deallocate_frames: bool,
) -> Result<(), UnmapError> {
    for page in range {
        let (frame, flusher) = mapper.unmap(page)?;
        if deallocate_frames {
...
fn load_segment(
    ...
    info!("Loading & mapping segment: virtual_addr={:#x}, file_size={:#x}, mem_size={:#x}, flags={:?}",
          segment.virtual_addr(), segment.file_size(), segment.mem_size(), segment.flags());
    ...
    if mem_size > file_size {
        ...
        for page in Page::range_inclusive(
            ...
            Page::containing_address(zero_end),
        ) {
            let frame = frame_allocator
                .allocate_frame()
                .ok_or(MapToError::FrameAllocationFailed)?;

...
impl Process {
    ...
    pub fn new(
        ...
        let proc_vm = proc_vm.unwrap_or_else(|| ProcessVm::new(PageTableContext::new(), false));
...
impl ProcessInner {
    ...
    pub fn kill(&mut self, ret: isize) {
        debug!("ProcessInner::kill called for process: {}", self.name);
        if let Some(vm) = &self.proc_vm {
            debug!("Before kill: page table using_count = {}", vm.page_table.using_count());
        }
    ...
    pub fn fork(&mut self, parent_weak_ref: Weak<Process>, parent_pid: ProcessId) -> ProcessInner {
        ...
        let parent_stack_start_virtaddr = self.vm().stack.start_address();
        
        let rsp_offset_from_stack_start = parent_rsp_virtaddr - parent_stack_start_virtaddr;
        
        let child_stack_start_virtaddr = child_vm.stack.start_address();
        ...
        let child_name = alloc::format!("{}-child-{}", self.name, parent_pid.0);

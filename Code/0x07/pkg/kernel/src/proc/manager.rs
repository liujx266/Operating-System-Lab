...
impl ProcessManager {
    ...
    pub fn spawn_kernel_thread(
        ...
        let kproc = match self.get_proc(&KERNEL_PID) {
            ...
            None => {
                ...
                let vm = ProcessVm::new(PageTableContext::new(), true).init_kernel_vm(&empty_kernel_pages);
        ...
        let proc_vm = Some(ProcessVm::new(page_table, true));
        ...
        inner.context.init_stack_frame(entry, VirtAddr::new(0));
        drop(inner); // 释放锁
    ...
    pub fn spawn(
        ...
        let page_table = kproc.read().clone_page_table();
        let proc_vm = Some(ProcessVm::new(page_table, false));
        ...
        // 设置栈帧
        inner.context.init_stack_frame(
            VirtAddr::new(elf.header.pt2.entry_point()),
            VirtAddr::new(vm::stack::STACK_INIT_TOP),
        );
    ...
    pub fn print_process_list(&self) {
        ...
        //      Calculate total memory usage from all processes
        let total_process_memory: u64 = self.processes
            .read()
            .values()
            .filter(|p| p.read().status() != ProgramStatus::Dead)
            .map(|p| p.read().vm().memory_usage())
            .sum();
        ...
        output += &format_usage("Memory", total_process_memory as usize, total);
        drop(alloc);

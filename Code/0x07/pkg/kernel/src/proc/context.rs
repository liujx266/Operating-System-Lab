...
impl ProcessContext {
    #[inline]
    pub fn as_mut(&mut self) -> Volatile<&mut ProcessContextValue> {
        Volatile::new(&mut self.value)
    }

    #[inline]
    pub fn as_ref(&self) -> Volatile<&ProcessContextValue, ReadOnly> {
        Volatile::new_read_only(&self.value)
    }
    ...
    pub fn save(&mut self, context: &ProcessContext) {
        // 使用正确的访问方式获取值
        self.value = context.as_ref().read();
    }

    #[inline]
    pub fn restore(&self, context: &mut ProcessContext) {
        // 使用正确的访问方式写入值
        context.as_mut().write(self.value);
    }
